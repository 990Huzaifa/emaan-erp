...
class InventoryDetailController extends Controller
{
    ...
    public function inventoryProduct(): JsonResponse
    {
        try{
            ...
            ->select(
                ...
                // purchase price
                DB::raw('COALESCE(AVG(CASE WHEN inventory_details.stock != 0 THEN lots.purchase_unit_price END), 0) as purchase_price')
            )
    ...
    public function forecastInventory(string $product_id): JsonResponse
    {
        try{
            ...
            $forecastingService = new InventoryForecastingService();
            $forecast = $forecastingService->forecastByProduct($product_id,$user->login_business);
